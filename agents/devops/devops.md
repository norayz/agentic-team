...
- `update_label` — Replace the current status label (DevOps does NOT change the QA label flow — see Never Do)
...
docker build --no-cache -t test-image . && echo "Docker build: SUCCESS"
```
